...
int main() {
    ...
    Matrix *stoch_unif = Stochastic_runif(5, 0, 10);
    ...
    printf("||error||_2 : %lf\n", Vector_distance(transition_applied, vec_stationary));
    ...
    Matrix *ehren = Matrix_from((double []) {0, 1, 0, 0, .5, 0, .5, 0, 0, .5, 0, .5, 0, 0, 1, 0}, 4, 4);

    Matrix_print(ehren);

    Matrix *ehren_stable = Matrix_pow(ehren, 20);

    Matrix_print(ehren_stable);

    Matrix_print(gausselim(ehren_stable, NULL));

    Matrix *rainy = Matrix_from((double []) {0.6, .4, .2, .8}, 2, 2);
    Matrix_print(Matrix_pow(rainy, 20));
